...
if(login_check($mysqli) == true){ 
    if(isset($_POST['submit'])){
        ...
        $alumn_id = sanitizeInput($alumn_id);
        $description = sanitizeInput($description);
        
        
        $stmt = $mysqli->prepare('UPDATE alumni SET section = ?, year_in = ?,
                year_out = ?, country = ?, city = ?, occupation_type = ?, occupation = ?, telephone = ?, mailing_subscription = ?, contact_agree = ?, description = ?
                WHERE alumn_id = ?');
        $stmt->bind_param('ssssssssssss', $section, $year_in, $year_out,
                $country, $city, $occupation_type, $occupation, $telephone, $mailing_subscription, $contact_agree, $description, $alumn_id);
        if($stmt->execute()){
            ...
            $group_member = $mysqli->prepare('SELECT * FROM int_alumni_groups WHERE alumn_id = ? AND group_id = ?');
            $group_member->bind_param('ss', $alumn_id, $group_id);
            $group_member->execute();
            $group_member->store_result();
            
            if($group_member->num_rows == 0){
            ...
            echo "<div class='alert alert-success'>Daten erfolgreich bearbeitet!<br>"
            . "<a href='index.php?path=pages/alumni/home'>Zur Alumni-Homepage...</a></div>";
        }
        
    }
    
    $alumn_id = $_GET['alumn_id'];
    $alumn_id = sanitizeInput($alumn_id);
    $get_alumn_data = $mysqli->prepare('SELECT section, year_in, year_out, country, city, occupation_type, occupation, telephone, mailing_subscription, contact_agree, description FROM alumni WHERE alumn_id = ?');
    $get_alumn_data->bind_param('s', $alumn_id);
    $get_alumn_data->execute();
    $get_alumn_data->store_result();
    $get_alumn_data->bind_result($section, $year_in, $year_out, $country, $city, $occupation_type, $occupation, $telephone, $mailing_subscription, $contact_agree, $description);
    $get_alumn_data->fetch();
    
?>

<a href='index.php?path=pages/alumni/home'>&laquo; Zurück zur Alumni-Homepage</a>
<h1>Profil bearbeiten</h1>
<p>So können Sie Ihre Alumni-Daten auf dem neusten Stand bringen!</p>
<br>
<div class="container col-sm-6 col-sm-offset-3">
    <table class="table table-bordered">
        <tr class="active">
            <td>
                <h3 class="text-center"><small>PROFILBEARBEITUNG</small></h3>
            </td>
        </tr>
        <tr>
            <td>
                <br>
                <form class="form-horizontal" id="alumniregform" action="index.php?path=pages/alumni/editprofile&alumn_id=<?php echo $alumn_id; ?>" method="post">
                    <input type="hidden" name="alumn_id" value="<?php echo $alumn_id ?>" />
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="year_in" placeholder="Notwendig" name="year_in" value="<?php echo $year_in; ?>" required>
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="year_out" class="col-sm-2 col-sm-offset-1 control-label">Abgangsjahr</label>
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="year_out" placeholder="" name="year_out" value="<?php echo $year_out; ?>">
                    </div>
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="city" placeholder="" name="city" value="<?php echo $city; ?>">
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <select class="form-control" id="occupation_type" name="occupation_type">
                            <option><?php echo $occupation_type; ?></option>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="occupation" placeholder="" name="occupation" value="<?php echo $occupation; ?>">
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="telephone" class="col-sm-2 col-sm-offset-1 control-label">Telefon-<br>nummer</label>
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="telephone" placeholder="" name="telephone" value="<?php echo $telephone; ?>">
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="description" class="col-sm-2 col-sm-offset-1 control-label">Über mich</label>
                      <div class="col-sm-8">
                          <textarea class="form-control" id="description" placeholder="Kurze Beschreibung" rows="4" name="description" maxlength="1000"><?php echo $description; ?></textarea>
                      </div>
                  </div>
            </td>
        </tr>
            ...
            <td>
                  <div class="form-group">
                    <div class="col-sm-offset-1 col-sm-10">
                      <div class="checkbox">
                        <label>
                          <input type="checkbox" name="mailing_subscription" value="YES" <?php if($mailing_subscription == 'YES') echo 'checked'; ?> /> Ich möchte regelmäßig spannende Neuigkeiten über die DSA per E-Mail bekommen.
                        </label>
                      </div>
                    </div>
                  </div>
            ...
            <td>
                  <div class="form-group">
                    <div class="col-sm-offset-1 col-sm-10">
                      <div class="checkbox">
                        <label>
                          <input type="checkbox" name="contact_agree" value="YES" <?php if($contact_agree == 'YES') echo 'checked'; ?> /> Ich möchte von anderen Alumni Mails bekommen (ohne Veröffentlichung Ihrer Adresse).
                        </label>
                      </div>
                    </div>
                  </div>
        ...
        <tr>
            <td>
                  <div class="form-group">
                    <div class="col-sm-offset-1 col-sm-10">
                      <button type="submit" class="btn btn-primary" name="submit" value="submit">Speichern</button>
                      <a class="btn btn-default" href="index.php?path=pages/alumni/editprofile&alumn_id=<?php echo $alumn_id; ?>" role="button">Reset</a>
                    </div>
                  </div>
                </form>
            </td>
